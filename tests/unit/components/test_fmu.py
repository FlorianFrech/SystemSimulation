...
class TestFMUComponentParameters:
    ...
    def test_set_parameters_before_init(self, pendulum_fmu_path):
        ...
        L_vr = comp._md_vars["L"].valueReference
        q0_vr = comp._md_vars["q0"].valueReference
    ...
    def test_set_parameters_unknown_name(self, pendulum_fmu_path):
        """Test setting an unknown parameter raises."""
        comp = FMUComponent("Pendulum", fmu_path=pendulum_fmu_path)

        with pytest.raises(KeyError):
            comp.set_parameters(does_not_exist=1.0)

    def test_get_parameters_unknown_name(self, pendulum_fmu_path):
        """Test requesting unknown parameters raises."""
        comp = FMUComponent("Pendulum", fmu_path=pendulum_fmu_path)

        with pytest.raises(KeyError):
            comp.get_parameters("does_not_exist")

    def test_set_parameters_unit_conversion(self, pendulum_fmu_path):
        """Test parameter unit conversion with Quantity."""
        comp = FMUComponent("Pendulum", fmu_path=pendulum_fmu_path)
        comp.set_parameters(L=Quantity(50.0, ureg("cm")))

        assert isinstance(comp.parameters["L"], Quantity)
        assert np.isclose(comp.parameters["L"].to("m").magnitude, 0.5)

    def test_set_parameters_unit_incompatible(self, pendulum_fmu_path):
        """Test incompatible parameter units raise errors."""
        comp = FMUComponent("Pendulum", fmu_path=pendulum_fmu_path)

        with pytest.raises(TypeError):
            comp.set_parameters(L=Quantity(1.0, ureg("s")))

    def test_set_parameters_int_bool_string(self, pendulum_fmu_path):
        """Test parameter validation for INT/BOOL/STRING types."""
        comp = FMUComponent("Pendulum", fmu_path=pendulum_fmu_path)

        comp._md_vars["int_param"] = SimpleNamespace(name="int_param", _python_type=int, unit=None)
        comp._md_vars["bool_param"] = SimpleNamespace(
            name="bool_param", _python_type=bool, unit=None
        )
        comp._md_vars["str_param"] = SimpleNamespace(name="str_param", _python_type=str, unit=None)
        comp.parameters["int_param"] = None
        comp.parameters["bool_param"] = None
        comp.parameters["str_param"] = None

        comp.set_parameters(int_param=3, bool_param=True, str_param="ok")

        assert comp.parameters["int_param"] == 3
        assert comp.parameters["bool_param"] is True
        assert comp.parameters["str_param"] == "ok"

    def test_set_parameters_int_wrong_type(self, pendulum_fmu_path):
        """Test INT parameter rejects invalid type."""
        comp = FMUComponent("Pendulum", fmu_path=pendulum_fmu_path)
        comp._md_vars["int_param"] = SimpleNamespace(name="int_param", _python_type=int, unit=None)
        comp.parameters["int_param"] = None

        with pytest.raises(TypeError):
            comp.set_parameters(int_param="bad")
    ...
    def test_reference_frequency_parameter(self, reference_fmu_path):
        ...
        freq_vr = comp._md_vars["frequency"].valueReference
        amp_vr = comp._md_vars["amplitude"].valueReference
...
class TestFMUComponentIO:
    ...
    def test_set_inputs_unknown_name(self, pendulum):
        """Test that unknown inputs raise errors."""
        with pytest.raises(KeyError):
            pendulum.set_inputs({"does_not_exist": 1.0}, t=0.0)

    def test_set_inputs_type_validation(self, pendulum):
        """Test that invalid input types raise errors."""
        with pytest.raises(TypeError):
            pendulum.set_inputs({"torque": "not-a-number"}, t=0.0)

    def test_set_inputs_before_initialize(self, pendulum_fmu_path):
        """Test that set_inputs before initialize raises."""
        comp = FMUComponent("Pendulum", fmu_path=pendulum_fmu_path)

        with pytest.raises(RuntimeError):
            comp.set_inputs({"torque": 1.0}, t=0.0)
    ...
    def test_evaluate_outputs_returns_raw_values(self, angle_encoder):
        """Test evaluate_outputs returns raw (non-Quantity) values."""
        outputs = angle_encoder.evaluate_outputs({"q": 0.2}, t=0.0)

        assert isinstance(outputs["U_q"], float)
...
class TestFMUComponentReset:
    ...
    def test_soft_reset_before_initialize_raises(self, pendulum_fmu_path):
        """Test soft_reset without initialization raises."""
        comp = FMUComponent("Pendulum", fmu_path=pendulum_fmu_path)
...
# ============================================================================
# Test FMUComponent Helpers
# ============================================================================
class TestFMUComponentHelpers:
    """Test module-level helper functions."""

    class _DummyVar:
        def __init__(self, name, py_type, unit=None):
            self.name = name
            self._python_type = py_type
            self.unit = unit

    def test_port_type_from_var_supported(self):
        """Test PortType detection for supported types."""
        assert _port_type_from_var(self._DummyVar("a", float)) == PortType.REAL
        assert _port_type_from_var(self._DummyVar("b", int)) == PortType.INT
        assert _port_type_from_var(self._DummyVar("c", bool)) == PortType.BOOL
        assert _port_type_from_var(self._DummyVar("d", str)) == PortType.STRING

    def test_port_type_from_var_unsupported(self):
        """Test PortType detection for unsupported types."""
        with pytest.raises(NotImplementedError):
            _port_type_from_var(self._DummyVar("e", bytes))

    def test_convert_start_value_none(self):
        """Test conversion of missing start value."""
        assert _convert_start_value(None, PortType.REAL, "m") is None

    def test_convert_start_value_real_with_unit(self):
        """Test conversion of REAL start value with unit."""
        value = _convert_start_value("2.5", PortType.REAL, "m")
        assert isinstance(value, Quantity)
        assert np.isclose(value.to("m").magnitude, 2.5)

    def test_convert_start_value_bool(self):
        """Test conversion of BOOL start value."""
        assert _convert_start_value("true", PortType.BOOL, None) is True
        assert _convert_start_value("0", PortType.BOOL, None) is False

    def test_convert_start_value_string(self):
        """Test conversion of STRING start value."""
        assert _convert_start_value("hello", PortType.STRING, None) == "hello"

    def test_in_vr_map_for_type(self, pendulum_fmu_path):
        """Test selection of input VR maps by port type."""
        comp = FMUComponent("Pendulum", fmu_path=pendulum_fmu_path)
        assert _in_vr_map_for_type(comp, PortType.REAL) is comp._vrs_in_real
        assert _in_vr_map_for_type(comp, PortType.INT) is comp._vrs_in_int
        assert _in_vr_map_for_type(comp, PortType.BOOL) is comp._vrs_in_bool
        assert _in_vr_map_for_type(comp, PortType.STRING) is comp._vrs_in_str
