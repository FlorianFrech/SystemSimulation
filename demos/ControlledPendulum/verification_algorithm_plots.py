...
ref_res = {'simple': ref_res_simple,
           'complex': ref_res_complex}
...
simple_dir = {'Jacobi': jacobi_dir_simple,
              'Gauss-Seidel': gs_dir_simple,
              'IJCSA': ijcsa_dir_simple}
...
complex_dir = {'Jacobi': jacobi_dir_complex,
               'Gauss-Seidel': gs_dir_complex,
               'IJCSA': ijcsa_dir_complex}

dirs = {'simple': simple_dir,
        'complex': complex_dir}
...
def create_trajectory_plot(case: str, monolithic_signal: str, cs_signal: str, y_label: str):
    ...
    fig.suptitle(f'Trajectory Comparison for {monolithic_signal}\nCase: {case.capitalize()}',
                 **subtitle_style, y=1.05)
...
def create_error_convergence_plot(case: str, monolithic_signal: str, cs_signal: str):
    ...
    ax.set_title(f'Error Convergence for {monolithic_signal}\nCase: {case.capitalize()}', **subtitle_style)
    ...
    ax.loglog(h_values, ref_line * (errors_dict['Jacobi'][0] / ref_line[0]),
              'k--', label=r'$O(\Delta t)$', linewidth=2)
    ax.loglog(h_values, ref_line_2nd * (errors_dict['Jacobi'][0] / ref_line_2nd[0]),
              'k:', label=r'$O(\Delta t^2)$', linewidth=2)
    ...
    plt.savefig(figures_dir / f'error_convergence_{cs_signal.replace(".", "_")}.svg', bbox_inches='tight')
    plt.savefig(figures_dir / f'error_convergence_{cs_signal.replace(".", "_")}.pdf', bbox_inches='tight')
    plt.close(fig)
