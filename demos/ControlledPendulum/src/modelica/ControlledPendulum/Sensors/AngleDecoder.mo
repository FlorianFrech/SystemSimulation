...
model AngleDecoder
  ...
  // Output
  Modelica.Blocks.Interfaces.RealOutput theta(unit="rad", start=0) "Quantized angle";
...
protected
  ...
  Integer code(start=0, fixed=true);
  Real v_quant(unit="V", start=0, fixed=true);
  Real pot_angle(unit="rad", start=0, fixed=true);
  Real theta_raw(unit="rad", start=0, fixed=true);
  discrete Real theta_hold(unit="rad", start=0, fixed=true);
