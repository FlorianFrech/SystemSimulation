...
Use the versioned OpenModelica User's Guide `v1.26.3` as the cited source for OpenModelica-specific solver descriptions, simulation options, and FMU export capabilities.
The corresponding bibliography key should be `openmodelica_users_guide_2026`.
Use this source when stating the standard DASSL behavior for monolithic reference execution and the CVODE or Euler export capabilities used by the generated FMUs.
...
```latex
The OpenModelica reference simulations use DASSL~\cite{openmodelica_users_guide_2026}.
...
```latex
The Co-Simulation FMUs use CVODE as their default internal solver~\cite{openmodelica_users_guide_2026}.
...
- **[Open] Missing space at sentence boundary.**
  `66_discussion_of_results.tex:47` reads
  *"...for the reported FEM model and contact scenario.For the reported
  configuration..."* — insert a space after the period.
- **[Open] Macro step notation `$\Delta t$` vs `$H_k$`.**
  Chapter 6 uses `$\Delta t$` for the macro step at
  `64_reference_model.tex:119`, `65_validation_scenarios.tex:24,39,40,65`,
  `66_discussion_of_results.tex:14`. Chapter 2 notation defines
  `$H_k$`. Either align Chapter 6 with `H_k` or add the dual-usage note
  to the Chapter 2 notation table (cheaper, since case-study scenarios
  read more naturally with `\Delta t = 10^{-3}\,\mathrm{s}`).
- **[Open] Short-form captions missing on §6.2–§6.4 tables.**
  Bare `\caption{...}` at `62_controlled_pendulum_system.tex:39`,
  `63_pendulum_models.tex:21`, `64_reference_model.tex:28`. Convert to
  `\caption[<short>]{<long>}` per `writing_style.md` §10/§21.

### Low Priority

- **[Open] §6.3.3 FEM-pendulum figure not yet inserted in the chapter.**
  The figure is recommended (mesh + contact deformation panels) and is
  being generated by `fem_mesh_export.ipynb`. The optional compliant-
  pendulum illustration is documented in the *Compliant-material
  illustration* subsection above. Insert when the notebook exports are
  final.
