...
\node[coord, below=4pt of pin] {$\eta = \theta$};

% Legend
\node[joint, scale=0.7] at (0.3,-1.3) {W};
\node[font=\footnotesize, anchor=west] at (0.7,-1.3) {WeldJoint ($n_\eta = 0$)};
\node[joint, scale=0.7] at (4.3,-1.3) {P};
\node[font=\footnotesize, anchor=west] at (4.7,-1.3) {PinJoint ($n_\eta = 1$)};
