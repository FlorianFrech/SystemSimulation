...
\draw pic[-{Latex[length=3,width=2]},thin,"$\theta$",thetaSignalColor,draw=thetaSignalColor,
...
\draw pic[->,"$\theta$", draw=thetaSignalColor, thetaSignalColor, angle radius=60, angle eccentricity=1.15] {angle=B--O--M};
...
\draw[<-, very thick, torqueSignalColor] (160:0.5) arc (160:70:0.5)
node[left=15pt] {$\tau$};
...
{Dynamics:
  \begin{align*}
    \dot{\theta} & = {\color{omegaSignalColor}\omega}                           \\
    \dot{\omega} & = \frac{{\color{torqueSignalColor}\tau} - mgL\sin(\theta)}{I}
  \end{align*}
  Rigid contact:
  \begin{align*}
    {\color{omegaSignalColor}\omega^+} &= -{\color{omegaSignalColor}\omega^-}
  \end{align*}
};
