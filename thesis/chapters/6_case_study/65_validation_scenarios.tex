...
The first two switches form a short FMU--OpenSim--FMU start-up transient.
The second OpenSim interval hands over to FEM shortly before the contact-relevant phase.
After the repeated contacts, the plant returns through OpenSim to FMU mode.

\begin{table}[htbp]
    \centering
    \small
    \caption[Errors for the multi-model contact scenario]{Trajectory and event-time errors for the multi-model contact scenario, computed against the interpolated OpenModelica rigid-contact reference.}
    ...
    \begin{tabular}{llrr}
        ...
        \(\theta\) (rad) & contact window \([0.15,0.35]\,\mathrm{s}\) & --- & \(1.571\cdot 10^{-3}\) \\
        \(t_e\) (s) & 5 paired contact events & \(3.729\cdot 10^{-4}\) & --- \\
        \bottomrule
    \end{tabular}
\end{table}
...
FEM activation is additionally locked out before \(t=0.10\,\mathrm{s}\) so that the controlled-pendulum upswing always runs on the rigid-body FMU and the first FEM activation coincides with the contact phase.
...
\begin{figure}[t]
    ...
    \caption[Performance benchmark of full-FEM versus switched FEM execution]{Performance benchmark of full-FEM versus switched FEM execution.
...
The FEM solver wall time decreases from \(458.3\,\mathrm{s}\) to \(222.1\,\mathrm{s}\), which corresponds to a reduction by \(2.06\times\).
