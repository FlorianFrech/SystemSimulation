...
The FMU pendulum is a rigid-body model.
...
Compliant penetration and spatial contact effects are assigned to the FEM pendulum.
...
Figure~\ref{fig:case_fem_model} gives the geometric and field context for the FEM variant.
It identifies the mesh, material domains, and boundary regions used by the model.
The stress panel illustrates that the FEM component contains spatial field quantities, although the co-simulation interface exposes only the projected angle and angular velocity.
...
\begin{figure}[t]
  ...
  \end{subfigure}
  \caption[FEM pendulum model used for the contact-capable plant variant]{FEM pendulum model used for the contact-capable plant variant.
...
The FEM model maps this scalar input to a distributed traction on the rotation boundary.
...
The surrounding co-simulation consumes the shared plant outputs rather than the full displacement field.
The FEM component projects the displacement and velocity fields onto rigid-body proxy quantities.
