...
The \ac{PID} controller computes the drive command.
...
In contact scenarios, this event triggers the pendulum contact response and the reset of the \ac{PID} integrator.
...
\begin{figure}[p]
  ...
  The pendulum plant is the interchangeable subsystem used by the \ac{FMU}, OpenSim, \ac{FEM}, and \texttt{MasterPendulum} variants.}
...
\begin{table}[tbp]
    \centering
    \small
    \caption[Subsystems of the controlled-pendulum case study]{Subsystems of the controlled-pendulum case study.}
    ...
    \begin{tabular}{p{3.0cm}p{5.0cm}p{5.6cm}}
        ...
        & Exchanged between \ac{FMU}, OpenSim, \ac{FEM}, and \texttt{MasterPendulum} variants \\
        \bottomrule
    \end{tabular}
\end{table}
...
This interface allows the \ac{FMU}, OpenSim, \ac{FEM}, and multi-model pendulum variants to be exchanged without changing the controller, drive, sensor, or decoder.
