...
In the average-acceleration form used here, the update relations are~\cite{newmark_method_1959,kochmann_introduction_2025}
\begin{align}
    \frac{\mathbf{U}^{n+1}-\mathbf{U}^n}{\Delta t}
    &=
    \frac{\dot{\mathbf{U}}^n+\dot{\mathbf{U}}^{n+1}}{2},
    \label{eq:fem_newmark_displacement}
    \\
    \frac{\dot{\mathbf{U}}^{n+1}-\dot{\mathbf{U}}^n}{\Delta t}
    &=
    \frac{\ddot{\mathbf{U}}^n+\ddot{\mathbf{U}}^{n+1}}{2}.
    \label{eq:fem_newmark_velocity}
\end{align}
These equations can be rearranged so that the unknown velocity and acceleration at the new time step are expressed in terms of the unknown displacement $\mathbf{U}^{n+1}$.
The time-discrete problem is therefore solved by first finding $\mathbf{U}^{n+1}$, typically with Newton's method, and then updating $\dot{\mathbf{U}}^{n+1}$ and $\ddot{\mathbf{U}}^{n+1}$ from the Newmark relations.
