...
It does not provide an exhaustive review.
It highlights the modeling approaches, tool classes, and orchestration challenges that motivate the framework developed in this thesis.
...
Tools such as Dymola, SimulationX, and OpenModelica provide solvers, graphical editors, and libraries based on the Modelica language and the Modelica Standard Library.

For tool interoperability, the \ac{FMI} standard has become particularly important \cite{FMI2.0}.
...
This makes \ac{FMI} a central basis for co-simulation workflows across tools \cite{gomes_co-simulation_2019-1}.
...
OpenSim is a widely used open platform for such models \cite{OpenSim_Delp,seth_opensim_2018}.
It supports articulated rigid-body systems with joints, biomechanical constraints, and muscle-tendon actuation.
It also provides workflows such as forward dynamics and inverse dynamics.
OpenSim is therefore relevant for human-device interaction, but OpenSim models usually require tool-specific integration in co-simulation workflows.
...
As a result, these models are usually integrated through custom wrappers or tool-specific interfaces rather than through direct \ac{FMI} export.
...
\ac{FMI} supports two main execution modes.
In Model Exchange, the importing environment provides the numerical solver and integrates the model equations.
In Co-Simulation, the \ac{FMU} keeps its own internal solver and is advanced only at communication points.
\ac{FMI}~3.0 extends the interface with scheduled execution and clock-based mechanisms for discrete-time behavior and event-oriented interaction \cite{FMI3.0}.

Several frameworks build on \ac{FMI} to orchestrate coupled \ac{FMU} execution.
OMSimulator provides an \ac{FMI}-based composition and execution environment with scripting and graphical support \cite{noauthor_omsimulator_nodate}.
...
These tools show that \ac{FMI}-based co-simulation is established at the level of standardized simulation-unit coupling.

Within the Python ecosystem, several libraries address \ac{FMI} at different abstraction levels.
FMPy supports loading, inspecting, validating, and simulating \acp{FMU} \cite{dassault_systemes__catia-systems_fmpy_2026}.
PyFMI provides a simulation-oriented Python interface for Model Exchange and Co-Simulation \acp{FMU} \cite{andersson_pyfmi_nodate}.
Research-oriented tools such as CoFmuPy show that lightweight Python-based co-simulation environments can be built around \acp{FMU} \cite{friedrich_cofmupy_nodate}.
These tools provide a mature basis for FMU-centric workflows.
...
The importing framework must detect the event, localize its time, restore or roll back subsystem states when necessary, execute event handling logic, synchronize affected simulation units, and resume the continuous simulation \cite{broman_determinate_2013}.
...
\ac{FMI}~3.0 adds concepts such as clocks, event mode, and scheduled execution \cite{FMI3.0}.
...
This leads to multi-model and multi-fidelity simulation approaches \cite{peherstorfer_survey_2018,fernandez-godino_review_2023}.
...
Region-based runtime model substitution has been proposed as a strategy for reducing computational cost.
