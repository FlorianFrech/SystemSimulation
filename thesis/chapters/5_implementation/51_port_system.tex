...
The port system implements the interface introduced in Section~\ref{sec:architecture_cosimcomponent}.
It also supports connection validation, value propagation, history recording, and event-port creation.
...
Table~\ref{tab:impl_port_types} lists the data types represented by the \texttt{PortType} enumeration.
Physical units are allowed only for \texttt{REAL} ports.
The \texttt{EVENT} type represents boolean trigger ports.
Event ports are created automatically for registered event indicators during component initialization, as described in Section~\ref{sec:impl_component_interface}.

\begin{table}[htbp]
  \centering
  \small
  \caption[Supported port data types]{Supported port data types in the \texttt{PortType} enumeration.}  \label{tab:impl_port_types}
  \begin{tabular}{lll}
    \toprule
...
Physical units are handled through Pint~\cite{grecco_pint_2026}.
...
\ac{FMI}~2.0 \acp{FMU} exported from OpenModelica may use compact unit strings such as \texttt{"m.s-2"} for $\mathrm{m}\,\mathrm{s}^{-2}$.
...
The representative cases in Table~\ref{tab:port_verification} cover type validation, unit compatibility, automatic conversion, and \ac{FMI} unit-string normalization.
...
\begin{table}[htbp]
  \centering
  \small
  \caption[Port verification cases]{Acceptance and rejection cases for port values and port connections.}  \label{tab:port_verification}
  \begin{tabular}{lll}
    \toprule
    ...
    Normalize \ac{FMI} unit string           & \texttt{"m.s-2"}   & parsed as $\text{m}\,\text{s}^{-2}$ \\
    \bottomrule
  \end{tabular}
\end{table}
