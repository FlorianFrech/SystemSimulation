...
It implements the system abstraction introduced in Section~\ref{sec:architecture_system_connections} and realizes \ref{req:SR-03-01} and \ref{req:SR-03-02} under \ref{req:UR-03}.
...
Registration enforces unique component names within the system.
...
Hybrid scenarios use \texttt{EventConnection} to route events from a source component to a target component.
The source port identifies the event indicator.
The destination port identifies the event input of the target component.
...
It also creates the corresponding \texttt{Event} subscription for the target component.
The subscription is stored on the target and is consumed by the hybrid algorithm during event dispatch.
...
The resulting execution order initializes the system generation by generation, as shown in Figure~\ref{fig:impl_system_initialization}.
...
\begin{figure}[t]
  ...
  The system prepares components, computes execution metadata, and initializes generations in structural order.}
...
The system rejects ambiguous or late structural changes before simulation starts.
