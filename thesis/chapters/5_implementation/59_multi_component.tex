...
Section~\ref{sec:architecture_multicomponent} introduced the architectural roles of the wrapper, mode selector, hysteresis, and state adapter.
This section describes how these roles are realized in the implementation.
...
The \pyfn{\_detect\_direct\_feedthrough()} method compares the feedthrough dictionaries of all registered models.
A mismatch is rejected during initialization.
The structural analysis of Section~\ref{sec:impl_structural} assumes fixed zero-delay behavior for every component during a simulation run.
The \texttt{MultiComponent} class can therefore switch the internal model only when the externally visible dependency structure remains unchanged.
...
Zero-duration calls are treated as output-refresh evaluations and are delegated to the current active model without mode selection.
...
Only then is the selector evaluated at the current simulation time.
...
\begin{figure}[t]
  ...
  \caption[Runtime switching sequence]{Runtime switching sequence of the \texttt{MultiComponent} class for a positive macro step. The wrapper applies the switching guards, transfers state and cached inputs on an accepted switch, and then delegates the step to the active model.}
...
\texttt{evaluate\_outputs()} temporarily disables mode switching, evaluates the active model for the supplied trial inputs, and copies the resulting values to the wrapper ports.
This prevents direct-feedthrough detection and IJCSA residual evaluations from changing the active model.
...
\begin{figure}[t]
  ...
  \caption[Two-mode integrator check for runtime mode switching]{Two-mode integrator check for runtime mode switching.
