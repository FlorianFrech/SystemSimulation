...
- outputs: ``q``, ``omega``, ``alpha`` (and optionally contact diagnostics)
...
- **Numerical transients**: expect discontinuities at switches; validate
  continuity of the shared outputs (``q``, ``omega``) and monitor energy where
  applicable.
...
- a reference generator provides ``q_ref``
