...
def _record_label_for_component(comp: CoSimComponent, execution_idx: int = -1) -> str:
    ...
    def is_event_port(port: PortSpec) -> bool:
        return port.type != PortType.REAL

    events_in_ports = [port.name for port in comp.input_specs.values() if is_event_port(port)]
    event_out_ports = [port.name for port in comp.output_specs.values() if is_event_port(port)]
    ...
    if len(outs) == 0:
        ...
        has_feedthrough = (
            port_name in comp.direct_feedthrough and comp.direct_feedthrough[port_name]
        )
        if is_event:
            label += f'<TD PORT="{port_name}"><FONT COLOR="#0b84ff"><B>{port_name}</B></FONT></TD>'
...
def _edge_unit_label(sys: System, conn: Connection) -> str | None:
    """Get unit label from destination port specification."""
...
class SystemGraphVisualizer:
    ...
    def visualize(self, filename: str = "system_graph", format: str = "svg") -> None:
        ...
        with self.dot.subgraph(name="cluster_legend") as legend:
            ...
            legend.node("legend_node", label=legend_label, shape="plaintext", fontsize="8")
        ...
        for event_conn in self.system.event_connections:
            ...
            label = ""  # No unit label for events
