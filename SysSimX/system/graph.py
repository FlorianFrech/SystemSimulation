...
def compute_execution_order(system: "System") -> None:
    ...
    system._scc_index = {name: mapping[name] for name in system.components.keys() if name in mapping}
...
def move_delayed_producers_to_last_generation(system: "System") -> None:
    ...
    delayed_producers = {
        name
        for name in system.components.keys()
        if is_delayed_producer(system, name)
    }
...
def collect_global_interface_unknowns(system: "System") -> Tuple[List[Tuple[str, str]], Dict[Tuple[str, str], Tuple[str, str]]]:
    """
